...
class GanTrain:
    ...
    def train_evaluate(self, G, D, train_generator=True, train_discriminator=True, norm_g=1, norm_d=1, limit=None):
        if G.invalid or D.invalid:  # do not evaluate if G or D are invalid
            logger.warning("invalid D or G")
            return
        if limit is None:
            limit = config.gan.batches_limit
    ...
    def evaluate_population(self, generators, discriminators, previous_generators, previous_discriminators,
        ...
        general_log[0] += [generator.genome.codify() + [generator.fitness()] for generator in generators]
        general_log[1] += [discriminator.genome.codify() + [discriminator.fitness()] for discriminator in discriminators]

        return np.array(general_log)
    ...
    def start(self):
        ...
        for generation in tqdm(range(config.evolution.max_generations-1)):
            ...
            data = self.evaluate_population(g_children, d_children, generators_population, discriminators_population, best_generators, best_discriminators)
